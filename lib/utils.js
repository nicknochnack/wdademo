...
const DOC_CATEGORY_DATA_INDEX = 0;
const ENTITY_DATA_INDEX      = 1;
...
const DATE_FILTER = 'DA';
const DOC_CATEGORY_FILTER = 'DC';
...
function formatData(data, passages, filterString) {
  var formattedData = {};
  var newResults = [];
  //console.log("data in formatData : "+JSON.stringify(data));
  data.results.forEach(function(dataItem) {
    // only keep the data we show in the UI
	  //Changes as per powercor project
    if(dataItem.enriched_text){
    var newResult = {
      id: dataItem.id,
      title: dataItem.extracted_metadata.subject,
      filetype: dataItem.extracted_metadata.file_type,
      // text: dataItem.extracted_metadata.file_type,
      author: dataItem.extracted_metadata.author,
      date: 'None', //dataItem.metadata.CreateDate,
      score: dataItem.result_metadata.score,
      discFileName: dataItem.extracted_metadata.filename,
      filename: dataItem.extracted_metadata.subject,
      fileSource: 'None',
      docCategory: 'NONE',
      entities: dataItem.enriched_text.entities ? dataItem.enriched_text.entities : [],
      highlight: {
        showHighlight: false,
        field: '',                // title or text
        passageScore: '0',
    ...
    if (passages.results) {
    	//console.log('Inside passages.results...');
      ...
      if (dataItem.highlight && dataItem.highlight.text) {
        ...
        if (dataItem.highlight.text.length > 0) {
          newResult.highlight.fullText = '';
          dataItem.highlight.text.forEach((section)=>{
            newResult.highlight.fullText += `<br/> "... ${section} ..."<br/><br/>`
          })
          newResult.highlight.fullText = newResult.highlight.fullText.replace(/<em>/g, '<hilite>').replace(/<\/em>/g, '</hilite>');
          newResult.highlight.showHighlight = true;
  ...
  formattedData.matching_results = data.matching_results;
